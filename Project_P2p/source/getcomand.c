...
static void fuction_send(char *index_string, char *message)
{
    ...
    if(Send_message_to_connect(index,message,"send") != SUCCESS)
    {
        fprintf(stderr,"\n send message error\n ");
        return ;
    }
...
static void fuction_connect(char *ip , char *Port_string)
{
    ...
    fprintf(stdout,"\n Connect successful ready to send data\n");
...
void fuction_terminal(char *index_string)
{
    if(is_number(index_string) != SUCCESS)
    {
        fprintf(stderr,"Format wrong!!\n");
    ...
    int index = atoi(index_string);
    printf("\n Terminal Index %d ",index);

    Send_message_to_connect(index, NULL ,"terminal");
    Tcp_terminal(index);
}
